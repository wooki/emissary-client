    ...
    while (checkNext.length > 0) {
      ...
      sourceLines = sourceLines.filter((line) => {
        if (
          ...
          const pathIndex = currentPath.findIndex((x) =>
            SameLine(x, checkLine),
          );
...
export function SameCoord(coord1, coord2, precision) {
  ...
  return (
    coord1 &&
    coord2 &&
    Math.round(coord1.x, precision) == Math.round(coord2.x, precision) &&
    Math.round(coord1.y, precision) == Math.round(coord2.y, precision)
  );
}

export function SameLine(line1, line2) {
  return (
    (SameCoord(line1[0], line2[0]) && SameCoord(line1[1], line2[1])) ||
